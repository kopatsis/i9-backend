...
func SaveDBAllAsync(user shared.User, newlevel float32, userExMod, userTypeMod map[string]float32, ratings [9]float32, workout shared.Workout, exerFacts map[string][3]float32, database *mongo.Database) (error, error, error) {
	...
	go func() {
		defer wg.Done()
		userErr = SaveUser(user, newlevel, userExMod, userTypeMod, database)
	}()
